...
1. clone 링크로 GET 요청을 보낸다.
2. GitHub에서 해당 요청이 Git에서 발생한 요청인지를 확인한다. 아니라면 에러를 반환하거나 실제 사이트 링크로 redirect 시킨다.
3. git이 보낸 요청이라면, 실제 Git 데이터를 반환한다.
4. git이 해당 데이터를 해석하여 .git 파일로 변환해준다.
